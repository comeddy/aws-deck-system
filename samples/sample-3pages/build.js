/**
 * sample-3pages/build.js — 3-page AWS Standard Deck sample (Cover + Content + Closing).
 *
 * Outputs: <repo>/output/sample-3pages.pptx
 * Run:    node samples/sample-3pages/build.js
 */
...
const OUTPUT_PATH = path.join(REPO_ROOT, "output", "sample-3pages.pptx");

const pres = new pptxgen();
pres.layout = "LAYOUT_16x9";
pres.author = "AWS Korea V-team";
pres.title  = "AWS Standard Deck — 3-page Sample";
...
// ───────────────────────────────────────────────────────────────────────
{
  ...
  s.addText("3-page Sample for Layout Verification", {
  ...
  s.addNotes(
    ...
    "오늘은 AWS 표준 프레젠테이션 디자인 시스템을 검증하기 위한 3페이지 샘플 deck을 보여드리겠습니다. " +
    "표지(cover), 콘텐츠 슬라이드(content), 마무리(closing) 세 가지 슬라이드 타입이 표준에 부합하는지 확인하는 것이 목표입니다."
  );
}
...
// ───────────────────────────────────────────────────────────────────────
// Slide 3 — Closing ("Thank you." — English-only, no bilingual)
// ───────────────────────────────────────────────────────────────────────
{
  const s = pres.addSlide();
  ...
  addFooter(s, ++pageNum);
  s.addNotes(
    "지금까지 AWS 표준 deck의 3페이지 샘플을 살펴보았습니다. " +
    "표지, 콘텐츠, 마무리 슬라이드의 디자인 토큰과 앵커가 모두 표준에 부합하는지 시각적으로 확인해 주시기 바랍니다. " +
